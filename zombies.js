...
Player.prototype.takeItem = function(item) {
  ...
  console.log(this.name + ': ' + item + ' was succesfully taken');
  return true;
};
/**
...
Player.prototype.equip = function(item) {
  const index = this.getPack().indexOf(item);
  if (index < 0 || !(item instanceof Weapon)) {
    return false;
  } else if (this.equipped) {
    const temp = this.equipped;
    this.equipped = item;
    this.getPack().splice(index, 1, temp);
  } else {
    this.equipped = item;
    this.getPack().splice(index, 1);
  }
...
Player.prototype.eat = function(itemToEat) {
  ...
  if (index < 0 || !(itemToEat instanceof Food)) {
    return false;
  }
